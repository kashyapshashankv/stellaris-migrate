        ...
        rollingMigrationPlans.forEach(plan => {
            if (plan.metadata?.name) {
                const filteredESXIMigrations = esxiMigrations.filter(esxi =>
                    esxi.metadata?.labels?.['vjailbreak.k8s.stellaris-migrate.io/rollingmigrationplan'] === plan.metadata.name
                );
        ...
        rollingMigrationPlans.forEach(plan => {
            if (plan.metadata?.name) {
                result[plan.metadata.name] = migrations.filter(migration =>
                    migration.metadata?.labels?.['vjailbreak.k8s.stellaris-migrate.io/rollingmigrationplan']?.includes(plan.metadata.name)
                );
            }
        });

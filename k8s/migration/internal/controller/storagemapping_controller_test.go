...
var _ = ginkgo.Describe("StorageMapping Controller", func() {
	ginkgo.Context("When reconciling a resource", func() {
		...
		ginkgo.BeforeEach(func() {
			ginkgo.By("creating the custom resource for the Kind StorageMapping")
			err := k8sClient.Get(ctx, typeNamespacedName, storagemapping)
			if err != nil && errors.IsNotFound(err) {
				resource := &vjailbreakv1alpha1.StorageMapping{
		...
		ginkgo.AfterEach(func() {
			...
			ginkgo.By("Cleanup the specific resource instance StorageMapping")
		...
		ginkgo.It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &StorageMappingReconciler{

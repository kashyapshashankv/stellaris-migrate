/* *************************************************
 * Copyright 2007-2021 VMware, Inc. All rights reserved. -- VMware Confidential
 * *************************************************/
...
static void DoCleanup(const VixDiskLibConnectParams *connectParams);
...
PrintUsage(void)
{
    ...
    printf(" -cleanup: perform cleanup firstly \n");
...
using VolumeInfoEx =
   std::unique_ptr<VixVolumeInfoEx, std::function<void(VixVolumeInfoEx *)>>;
...
DoMntApi()
{
   ...
   cout << "DiskSet Info - flags " << diskSetInfo->openFlags << " (passed - "
        << appGlobals.openFlags << "), mountPoint "
        << (diskSetInfo->mountPath == NULL ? "<null>" : diskSetInfo->mountPath)
        << "." << endl;
   ...
   for (auto i = j-1; i < (int)numVolumes; ++i) {
      ...
      VixVolumeInfoEx *volInfo = NULL;
      err = VixMntapi_GetVolumeInfoEx(volumeHandles[i], &volInfo);
      if (VIX_FAILED(err)) {
         VixDiskLibErrWrapper errWrap(err, __FILE__, __LINE__);
         cout << "Error: " << errWrap.Description() << endl;
         continue;
      }
      viset.emplace_back(volInfo, VixMntapi_FreeVolumeInfoEx);
      cout << "\nMounted Volume " << i + 1 << ", Type " << volInfo->type
           << ", isMounted " << volInfo->isMounted << ", symLink "
           ...
           << ((volInfo->numGuestMountPoints == 1)
                  ? (volInfo->inGuestMountPoints[0])
                  : "<null>")
           << ")\n"
           << "bytesPerSector " << volInfo->bytesPerSector
           << ", capacityInBytes " << volInfo->capacityInBytes << ", usedSpace "
           << volInfo->usedSpace << ", clusterSizeInBytes "
           << volInfo->clusterSizeInBytes << endl;
   }
   char isUnmount;
   do {
...
static void
DoCleanup(const VixDiskLibConnectParams *connectParams)
{
   uint32 numCleanedUp;
   uint32 numRemaining;
   ...
   CHECK_AND_THROW(vixError);
   cout << "num of VMs cleaned up: " << numCleanedUp
        << "num of VMs remaining: " << numRemaining << endl;
}
